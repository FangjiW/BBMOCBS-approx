...
bool ApexPathPair::update_nodes_by_merge_if_bounded(const ApexPathPairPtr &other, const std::vector<double> eps, MergingStrategy s){
  ...
  // choose a path node
  if (s == MergingStrategy::SMALLER_G2 || s == MergingStrategy::SMALLER_G2_FIRST){
    ...
    if (! is_bounded(new_apex, new_path_node, eps)){
      if (s == MergingStrategy::SMALLER_G2_FIRST && is_bounded(new_apex, other_path_node, eps)){
        new_path_node = other_path_node;
      }else{
        return false;
      }
    }
  }else if (s == MergingStrategy::RANDOM){
  ...
  }else if (s == MergingStrategy::MORE_SLACK){
    if (is_bounded(new_apex, this->path_node, eps)){
      new_path_node = this->path_node;
    }
    if (is_bounded(new_apex, other->path_node, eps)){
  ...
  }else if (s == MergingStrategy::REVERSE_LEX){
    new_path_node = this->path_node;
  ...
  }else if(s == MergingStrategy::CONFLICT_BASED){
    if(this->path_node->conflict_num < other->path_node->conflict_num){
      ...
      return this->update_nodes_by_merge_if_bounded(other, eps, MergingStrategy::MORE_SLACK);
    }
  }
  else{
