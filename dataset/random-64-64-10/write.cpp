...
int main(){
	ifstream input("random-64-64-10.map");
	std::string temp;
    int height, width;
	...
	for(int i = 0; i < 300; i++){
		ofstream output("cost2/random-" + to_string(i+1) + ".cost");
	...
	for(int i = 1; i < 11; i++){
		ofstream output2("cost2/unique" + std::to_string(i) + ".cost");
		for(int j = 0; j < height; j++){
			for(int k = 0; k < width; k++){
				output2 << i << " ";
			}
	...
	ofstream output3("cost2/distance.cost");
	for(int j = 0; j < height; j++){
		for(int k = 0; k < width; k ++){
			if(map[j][k] == -1){
				output3 << 0 << " ";
